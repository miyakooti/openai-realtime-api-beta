...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    ...
    this.apiKey = "sk-svcacct-xSTMUSEKVcE9z5ppziDALxNkN4jMHhwtPgt7tvoxuguqVBFlY-9XjT0YvP-56hVtcAOuaaT3BlbkFJbpYqZx0DBOP1KID4jvx_JhDaVgugd9SPBPV8wvuQaeL2p80gHEmDkjwjW_tbUYmKVPWs8A";
    this.debug = !!debug;
    this.ws = null;
  ...
  async connect({ model } = { model: 'gpt-4o-realtime-preview-2024-12-17' }) {
    if (!this.apiKey && this.url === this.defaultUrl) {
      console.warn(`No fjeawio apiKey provided for connection to "${this.url}"`);
    }
    if (this.isConnected()) {
